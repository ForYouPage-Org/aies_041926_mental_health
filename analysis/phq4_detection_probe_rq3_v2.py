#!/usr/bin/env python3
"""RQ3 Detection probe v2: per-item PHQ-2 / GAD-2 anchored prompting cascade.

Methodology grounded in:
  - LlaMADRS (Galatzer-Levy et al., 2025): per-item descriptive+demonstrative
    cue prompts, summed across items; ICC=0.96 with clinicians on MADRS.
  - eRisk 2024-2025 (Parapar et al.): symptom-by-symptom decomposition is the
    winning paradigm for depression detection.
  - Cognitive-Mental-LLM (Sadeghi et al., 2025): CoT helps binary scoring,
    deterministic aggregation across items.
  - Mental-LLM (Xu et al., 2024) / MentaLLaMA (Yang et al., 2024): researcher
    persona + IRB framing reduces refusal in safety-tuned models.

Cascade per participant (4 LLM calls):
  - PHQ-2 item 1 (anhedonia)  -> 0/1/2/3
  - PHQ-2 item 2 (low mood)   -> 0/1/2/3
  - GAD-2 item 1 (nervousness) -> 0/1/2/3
  - GAD-2 item 2 (worry)       -> 0/1/2/3

Each prompt has:
  - Researcher/coder persona + IRB disclaimer (refusal mitigation)
  - PHQ-4 item text VERBATIM (Kroenke 2009 wording)
  - Operational definition (what to look for in everyday LLM prompts)
  - 4 synthetic anchored exemplars (one per scale point 0-3) -- generated
    a-priori, not real participant data
  - Chain-of-thought: identify cues first, then output integer score
  - Per-item 0-3 score; PHQ-2 = sum of items 1-2 (0-6), GAD-2 = sum (0-6)

Then: classifiers (LIWC baseline, LLM cascade, combined) with 5x5 CV,
bootstrap AUC CIs, calibration, top-k recall, ROC plots.
"""
...
ap.add_argument("--n-msgs-per-participant", type=int, default=60)
ap.add_argument("--min-words-per-msg", type=int, default=4)
...
ap.add_argument("--out-suffix", default="phq4_detection_probe_v2")
args = ap.parse_args()
...
# ============================================================
def get_cascade_scores(liwc, prompts, args, cache_file: Path) -> pd.DataFrame:
    if args.cached_llm:
...
llm_df = get_cascade_scores(liwc, prompts, args, cache_file)
